...
func Login(h *handlers.Handlers) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if passOk := auth.CheckPasswordHash(customer.PasswordHash, params.Password); !passOk {
			apiutils.RespondWithError(h.ZapLogger, w, http.StatusBadRequest, "Invalid email or password")
			return
		}

		token, err = auth.GenerateJWT(customer.ID, "customer", auth.ConfigKey.CustomerJWTKey, auth.CustomerIssuer)
		if err != nil {
